...
public class Trie {
	...
	public void insert(String key) {
		...
		TrieNode current = getPrefixNode(key);
		for(int i=current.depth; i<key.length(); i++){
			current = current.createChild(key.charAt(i));
			if(i==key.length()-1){
	...
	private TrieNode getPrefixNode(String key) {
		...
		while ( counter < key.length()){
			if (current.getChild(key.charAt(counter)) == null)
				return current;
			else
				current = current.getChild(key.charAt(counter));
			counter++;
		}
		return current;

		//return end;  //  REPLACE THIS STUB
		//
	...
	public ArrayList<String> getAllPrefixMatches(String prefix) {
		...
		//  ADD YOUR CODE BELOW 
		TrieNode current;
		current = getPrefixNode(prefix);
		if(current.depth == prefix.length())
			getAllMatchesHelper(current, stringList);
...
//Recursive method based on the traverse method taught in class so that TrieNode t can be traversed

	public void getAllMatchesHelper(TrieNode node, ArrayList<String> list) {
		if (node != null && node.depth != 0) {
			...
			for (int i = 0; i < 256; i++) {
				if(node.getChild((char) i)!=null){
					getAllMatchesHelper(node.getChild((char) i), list);
				}
			}
		}
	}
	 ...
	 * 
	 * References:
	 * -For all unicode characters, see http://unicode.org/charts
